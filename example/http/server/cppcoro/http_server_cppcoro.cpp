...
public:
        ...
        ConstBufferSequence const& buffers)
    {
        invoked = true;
        task_ =
            [this, buffers]() -> cppcoro::task<error_code>
            {
                error_code ec;
                auto const mb = buffers_front(buffers);
                co_await stream_.send(mb.data(), mb.size());
                return ec;
            }();
    }
...
public:
    ...
    template<bool isRequest, class Body, class Fields>
    void
    operator()(http::message<isRequest, Body, Fields>&& m_)
    {
        task_ =
            [this, &m_]() -> cppcoro::task<beast::error_code>
            {
                beast::error_code ec;
                http::message<isRequest, Body, Fields> m(std::move(m_));
                http::serializer<isRequest, Body, Fields> sr(m);
                while(! sr.is_done())
                {
                    ec = co_await write_some(stream_, sr);
                    if(ec)
                        break;
                }
                return ec;
...
)
{
    ...
    for(;;)
    {
        ...
        co_await write.wait(); // small hack because operator co_await don't chain
    }
}
